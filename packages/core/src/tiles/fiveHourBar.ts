...
export function projectCap(
  ...
  resetsAt: number,
  nowSec: number,
  opts = { minElapsed: 600, minPct: 2, maxEta: 24 * 3600 }
  ...
  if (rate <= 0) return { resetsIn, capEta: null };
  const eta = (100 - usedPct) / rate;
  if (eta <= 0 || eta > opts.maxEta) return { resetsIn, capEta: null };
  return { resetsIn, capEta: humanDelta(eta) };
}

export const fiveHourBarTile: TileModule<{ width: number; showReset: boolean; showProjection: boolean }> = {
  id: "five-hour-bar",

...
export function barColor(pct: number): string {
  if (pct < 60) return "#87d787";
  if (pct < 80) return "#d7af5f";
  if (pct < 92) return "#ffaf5f";
  return "#ff5f5f";
}

export function renderBar(pct: number, width: number) {
...
export const contextBarTile: TileModule<{ width: number; showTokens: boolean; warnAt: number }> = {
  id: "context-bar",
  ...
  render(props, { cc }, mode) {
    ...
    if (pct >= props.warnAt) out.push(span("  ⚠ compact soon", { fg: "#ff5f5f" }));
    return out;
  },
};

...
function inRanges(cp: number, ranges: typeof WIDE_RANGES): boolean {
...
export function graphemeWidth(g: string): number {
  ...
  if (cps.includes(VARIATION_SELECTOR_15)) return 1;
  // Emoji-presentation selector forces the wide form.
  if (cps.includes(VARIATION_SELECTOR_16)) return 2;

  const base = cps[0]!;
  if (ZERO_WIDTH.has(base)) return 0;
  if (isCombining(base)) return 0;
  if (base < 0x20 || (base >= 0x7f && base < 0xa0)) return 0; // control chars
  if (inRanges(base, WIDE_RANGES)) return 2;
  return 1;
}
...
export function displayWidth(s: string): number {
  ...
  let w = 0;
  if (segmenter) {
    for (const { segment } of segmenter.segment(plain)) w += graphemeWidth(segment);
  } else {
    for (const ch of plain) w += graphemeWidth(ch);
  }
  return w;
}

/** Truncate to `cols` display columns, appending `ellipsis` if it had to cut. */
export function truncateToWidth(s: string, cols: number, ellipsis = "…"): string {
  if (cols <= 0) return "";
  if (displayWidth(s) <= cols) return s;
  const budget = cols - displayWidth(ellipsis);
  if (budget <= 0) return ellipsis.slice(0, cols);
  let out = "";
  let w = 0;

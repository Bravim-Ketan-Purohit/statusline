...
test("data-dependent tiles render nothing on empty data (no empty boxes)", () => {
  for (const [id, mod] of registry) {
    if (DATA_INDEPENDENT.has(id)) continue;
    assert.equal(mod.render(mod.defaultProps, empty, "full").length, 0,
      `${id} should render nothing with no data`);
  }
});
...
test("context bar percentage is column-stable from 0 to 100", () => {
  const mod = registry.get("context-bar")!;
  const widths = [0, 5, 42, 100].map((p) =>
    mod.render({ width: 8, showTokens: false, warnAt: 200 },
      { cc: { context_window: { used_percentage: p } }, local: {}, columns: 200 }, "full")
